...
namespace Reimaginate.DataHub.Agent.BusinessCentral.Services.DataHubEntityCache;
...
public class DataHubEntityCache : IDataHubEntityCache
{
    private readonly IDataHubClient _dataHubClient;
    private Dictionary<string, object> Cache { get; set; }

    public DataHubEntityCache(IDataHubClient dataHubClient)
    {
    ...
    public async Task<List<TDataHub>> GetDataHubEntities<TDataHub>(List<string> entityIds, CancellationToken cancellationToken, bool? retryNulls = false) where TDataHub : DataHubEntity
    {
        ...
        {
            var getDataHubEntitiesResponse = await _dataHubClient.PostRequestAsync<GetDataHubEntitiesByIdRequest, GetDataHubEntitiesByIdResponse>(new GetDataHubEntitiesByIdRequest()
            {
            ...
            {
                Cache.Add($"{typeof(TDataHub).Name}.{result.id}", result);
            }
            ...
            if (retryNulls == false)
            {
                ...
                {
                    Cache.Add($"{typeof(TDataHub).Name}.{entityId}", null);
                }
            }
        }
    ...
    public async Task<List<JObject>> GetDataHubEntities(string entityType, List<string> entityIds, CancellationToken cancellationToken, bool? retryNulls = false)
    {
        ...
        if (notFoundInCache.Any())
        {
            ...
            foreach (var result in results)
            {
                Cache.Add($"{entityType}.{result.Value<string>(nameof(DataHubEntity.id))}", result);
            }
            ...
            if (retryNulls == false)
            {
                ...
                foreach (var entityId in stillNotFound)
                {
                    Cache.Add($"{entityType}.{entityId}", null);
                }
            }
        }
    ...
    public void InvalidateCacheEntries<TDataHub>(List<string> entityIds) where TDataHub : DataHubEntity
    {
        ...
        cacheEntries.ForEach(entry => Cache.Remove(entry.Key));
    }
